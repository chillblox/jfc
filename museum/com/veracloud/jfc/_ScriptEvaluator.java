...
/**
 * Creates and compiles the given Java code block, wrapped into a Java method
 * with the given return type and parameter types, along with a Java class
 * definition that contains the given import statements.
 * <p>
 * Compilation is done in main memory, i.e. without writing to the file system.
 * <p>
 * The result is an object that can be executed (and reused) any number of
 * times.
 * <p>
 * This is a high performance implementation, using an optimized variant of
 * "https://scripting.dev.java.net/ (JSR 223 Java Scripting). Calling
 * {@link #evaluate(Object...)} just means calling
 * {@link Method#invoke(Object, Object...)} and as such has the same minimal
 * runtime cost.
 * <p>
 * Instances of this class are thread-safe if the user provided script
 * statements are thread-safe.
 */
@Deprecated
public class _ScriptEvaluator<T> {
  private static final Logger logger = Logger.getLogger(_ScriptEvaluator.class.getName());
  ...
  public _ScriptEvaluator(Class<?>[] javaImports, String javaCodeBlock, Class<T> returnType,
  ...
  public _ScriptEvaluator(Class<?>[] javaImports, String javaCodeBlock, Class<T> returnType,
    ...
    if (javaImports != null && javaImports.length > 0) {
      ...
      for (Class<?> javaImport : javaImports) {
        script += "\nimport " + javaImport.getCanonicalName() + ";";
      }
    }
  ...
  public T evaluate(Object... params) throws ScriptException {
    // TODO: consider restricting permissions/sandboxing; also see
    // http://worldwizards.blogspot.com/2009/08/java-scripting-api-sandbox.html
    try {
